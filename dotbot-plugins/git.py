...
class DotbotGit(dotbot.Plugin):
    ...
    def handle(self, directive, data):
        if directive != self._directive:
            raise ValueError("git cannot handle directive %s" % directive)
        success = True
        ...
        for clone_path in data:
            ...
            if "method" in repo_definition:
                if repo_definition["method"] in ["clone", "pull", "clone-or-pull"]:
                    ...
                    self._log.warning(
                        "Method "
                        + repo_definition["method"]
                        + "is not defined! (Will be ignored...)"
                    )
            if "description" in repo_definition:
                repo.description = repo_definition["description"]

            repo.quiet = repo_definition.get("quiet", defaults.get("quiet", repo.quiet))
            repo.shallow = repo_definition.get(
                "shallow", defaults.get("shallow", repo.shallow)
            )
    ...
    class Repository:
        ...
        def load(self):
            ...
            if method == "clone":
                if self._repo_exists():
                    method = None
                    self._log.lowinfo(
                        "Repository already exists! Won't clone "
            ...
            if method == "pull":
                if not self._repo_exists():
                    method = None
                    self._log.error(
                        "Repository doesn't exist! Woun't pull "
                        + self._get_description()
                    )
        ...
        def _run_command(self, command):
            try:
                subprocess.run([command], shell=True, check=True, stdout=not self.quiet)
                return True
